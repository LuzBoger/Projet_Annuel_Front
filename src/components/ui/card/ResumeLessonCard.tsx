...
export function ResumeLessonCard({ lesson }: ResumeLessonCardProps) {
    ...
    if (!lesson) {
        return (
            <div className="bg-white rounded-xl border border-gray-200 p-4">
                <p className="text-sm text-gray-400 uppercase tracking-widest mb-2">{t("lesson.resume")}</p>
                <p className="text-sm text-gray-500">{t("lesson.start_first_lesson")}</p>
            </div>
        );
    }
    ...
    return (
           <div className="bg-white rounded-xl border border-gray-200 p-4">
            <p className="text-xs text-gray-400 uppercase tracking-widest mb-3">{t("lesson.resume")}</p>

            <div className="flex items-center gap-2 mb-2">
                <LanguageFlag languageCode={lesson.languageCode} className="w-6 h-4 object-cover rounded-sm" />
                <span className="text-xs text-gray-400 uppercase">{lesson.topicName}</span>
            </div>

            <p className="font-semibold text-gray-900 text-base mb-1">{lesson.lessonName}</p>
