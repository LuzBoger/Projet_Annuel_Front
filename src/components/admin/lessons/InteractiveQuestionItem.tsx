import { Control, useFieldArray, UseFormRegister, useWatch, UseFormSetValue, FieldErrors } from "react-hook-form";
...
import { InteractiveQuestionError } from "@/types/lesson/lesson";

interface InteractiveQuestionItemProps {
  index: number;
...
export function InteractiveQuestionItem({
  index,
  control,
  register,
  errors,
    ...
    <div className="relative p-6 bg-gray-50 dark:bg-gray-900/40 rounded-xl border border-gray-100 dark:border-gray-800 group transition-all hover:bg-white dark:hover:bg-gray-900/60 hover:shadow-md">
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <FormField
            label={t("admin.lessons.interactive.questionText")}
            placeholder={t("admin.lessons.interactive.questionTextPlaceholder")}
          ...
          <div>
            <label className="text-sm font-semibold text-gray-900 dark:text-gray-200 block mb-1">
              {t("admin.lessons.interactive.systemType")}
            </label>
            <Select
              ...
              options={[
                { value: "MULTIPLE_CHOICE", label: t("admin.lessons.interactive.multipleChoice") },
                { value: "OPEN_TEXT", label: t("admin.lessons.interactive.openText") }
              ]}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 bg-white dark:bg-gray-800/50 rounded-xl border border-gray-150 dark:border-gray-800">
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-sm font-semibold text-gray-900 dark:text-gray-200">
                {t("admin.lessons.interactive.imagesLabel")}
              </label>
              <input
                type="file"
                accept="image/*"
              ...
              >
                ...
                {t("admin.lessons.interactive.addImage")}
              </Button>
            </div>
            ...
            <div className="grid grid-cols-4 gap-2">
              {imagePaths.map((path, imgIdx) => (
                ...
                >
                  ...
                  <Button
                    ...
                    className="absolute top-1 right-1 p-1 bg-red-600 text-white rounded-full opacity-0 group-hover/img:opacity-100 transition-opacity shadow flex items-center justify-center"
                  >
              ...
              {imagePaths.length === 0 && (
                <div className="col-span-4 py-6 text-center text-xs text-gray-400 border border-dashed border-gray-200 dark:border-gray-700 rounded-lg">
                  {t("admin.lessons.interactive.noImage")}
                </div>
              )}
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-sm font-semibold text-gray-900 dark:text-gray-200">
                {t("admin.lessons.interactive.audiosLabel")}
              </label>
              <input
                type="file"
                accept="audio/*"
              ...
              >
                ...
                {t("admin.lessons.interactive.addAudio")}
              </Button>
            </div>
            ...
            <div className="space-y-1.5">
              {audioPaths.map((path, audIdx) => (
                ...
                >
                  ...
                  >
                    <PlayIcon className="w-3 h-3 text-brand-600 dark:text-brand-400" />
              ...
              {audioPaths.length === 0 && (
                <div className="py-6 text-center text-xs text-gray-400 border border-dashed border-gray-200 dark:border-gray-700 rounded-lg">
                  {t("admin.lessons.interactive.noAudio")}
                </div>
              )}
            </div>
          </div>
        ...
        {systemType === "MULTIPLE_CHOICE" ? (
          <div className="space-y-3 pt-4 border-t border-gray-100 dark:border-gray-800">
            <div className="flex items-center justify-between mb-2">
              <label className="text-sm font-semibold text-gray-900 dark:text-gray-200">
                {t("admin.lessons.interactive.optionsLabel")}
              </label>
              <Button
                type="button"
                variant="pill-brand"
              ...
              >
                {t("admin.lessons.interactive.addOption")}
              </Button>
            </div>
          ...
          <div className="pt-4 border-t border-gray-100 dark:border-gray-800">
            <FormField
              label={t("admin.lessons.interactive.correctWordLabel")}
              placeholder={t("admin.lessons.interactive.correctWordPlaceholder")}
